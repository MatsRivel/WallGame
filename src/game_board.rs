...
impl GizmoStructBundle{
    pub fn new(gs: GizmoStruct, materials: &mut ResMut<Assets<StandardMaterial>>, meshes: &mut ResMut<Assets<Mesh>>)->Self{
        ...
        let mesh = match gs.grid_type{
            GridType::Tile => {
                let shape = Cuboid::from_corners(gs.vec() - Vec3::new(TILE_WIDTH/2f32, TILE_WIDTH/2f32, 0.0), gs.vec() + Vec3::new(TILE_WIDTH/2f32, TILE_WIDTH/2f32, 1.0));
                meshes.add(shape)
            },
            GridType::Cirle => {
            ...
            GridType::Horizontal => {
                let shape = Cuboid::from_corners(gs.vec() - Vec3::new(TILE_WIDTH/2f32, TRENCH_WIDTH/2f32, 0.0), gs.vec() + Vec3::new(TILE_WIDTH/2f32, TRENCH_WIDTH/2f32, 1.0));
                meshes.add(shape)
            },
            GridType::Vertical => {
                let shape = Cuboid::from_corners(gs.vec() - Vec3::new(TRENCH_WIDTH/2f32, TILE_WIDTH/2f32, 0.0), gs.vec() + Vec3::new(TRENCH_WIDTH/2f32, TILE_WIDTH/2f32, 1.0));
                meshes.add(shape)
            },
        };
